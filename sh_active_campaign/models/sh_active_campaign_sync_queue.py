...
class ShActiveCampaignSyncQueue(models.Model):
    ...
    @api.model
    def _commit_queue_progress(self, processed_count=0):
        """Advance cron progress when the core Odoo version supports it."""
        cron_model = self.env['ir.cron']
        commit_progress = getattr(cron_model, '_commit_progress', None)
        if callable(commit_progress):
            return commit_progress(processed_count)
        return True
    ...
    def process_queue(self, batch_size=100):
        ...
        while True:
            ...
            if from_cron:
                time_left = self._commit_queue_progress(len(queue_records))
                self.env.cr.commit()
                if not time_left:
                    break
            else:
    ...
    def action_retry(self):
        ...
        self.write({'sh_state': 'draft', 'sh_error_message': False})
        return {
    ...
    def process_queue_records(self, queue_records):
        ...
        for queue_rec in queue_records:
            try:
                ...
                _logger.exception('Unexpected queue processor error for job %s: %s', queue_rec.id, exc)
                queue_rec.write({
                    'sh_state': 'failed',
                    'sh_error_message': str(exc),
                })
