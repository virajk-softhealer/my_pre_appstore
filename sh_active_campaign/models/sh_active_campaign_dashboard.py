...
class ShActiveCampaignDashboard(models.Model):
    ...
    def _parse_ac_datetime(self, value):
        """Parse common ActiveCampaign date formats into a naive UTC datetime."""
        if not value:
            return False
        if isinstance(value, datetime):
            ...
            try:
                ...
                try:
                    parsed = fields.Datetime.from_string(value.replace('T', ' '))
                except Exception:
                    return False
        if parsed.tzinfo:
    ...
    def _is_ac_record_in_date_window(self, record, date_from, date_to, candidate_keys):
        """Return True when an AC record falls inside the selected date window."""
        if not (date_from or date_to):
            return True
        for key in candidate_keys:
            record_dt = self._parse_ac_datetime(record.get(key))
            if record_dt:
    ...
    def _sync_contacts_from_ac(self):
        ...
        use_date_range, date_from, date_to, from_queue_snapshot = self._get_sync_date_window()
        date_params = {}
        if use_date_range and date_from:
            date_params['filters[updated_after]'] = date_from.strftime('%Y-%m-%dT%H:%M:%S+00:00')
        else:
    ...
    def _sync_list_contacts_from_ac(self):
        ...
        try:
            ...
            data = client.get('/api/3/lists', params={'limit': 100, **date_params})
            lists = data.get('lists', [])
            if use_date_range:
                lists = [
        ...
        try:
            ...
            if contact_data:
                ...
                vals = {
                    ...
                    'email': email,
                    'phone': phone,
                    'sh_ac_contact_id': int(contact_id),
                }
